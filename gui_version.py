...
class InvoiceProcessor():
    ...
    def prompt_for_filename(self, title):
        root = tk.Tk()
        root.withdraw()  # Hide the main window
        filename = filedialog.askopenfilename(title=title)
        return filename

    def prompt_for_yes_no(self, title, message):
        root = tk.Tk()
        root.withdraw()  # Hide the main window
        return messagebox.askyesno(title=title, message=message)

    def process_invoices(self):
        df = pd.read_csv(self.prompt_for_filename('Enter location of Looker Export'))
        directory = pd.read_csv(self.prompt_for_filename('Enter location of Affiliate Directory'))
        ...
        increment_directory = self.prompt_for_yes_no("Increment Directory", "Would you like an incremented version of the directory?")
